...
Or use a `.env` file in the program's own directory (parsed automatically by
each program, relative to where you run it):

```bash
# go/ontap/<use_case>/.env
ONTAP_HOST=10.0.0.1
ONTAP_USER=admin
...
`go/ontap/ontapclient/`. Never build a new HTTP client in program code.
...
cd go/ontap/cluster_info && go run .
```
...
cd go/ontap/nfs_provision && go run .
```
...
cd go/ontap/cifs_provision && go run .
```
...
cd go/ontap/cluster_setup_basic && go run .
```
...
cd go/ontap/snapmirror_provision_src_managed && go run .
```
...
export DEST_SVM=vs1
export SM_POLICY=Asynchronous
cd go/ontap/snapmirror_provision_dest_managed && go run .
```
...
cd go/ontap/snapmirror_test_failover && go run .
```
...
cd go/ontap/snapmirror_cleanup_test_failover && go run .
```
...
- Environment variables loaded via `loadDotEnv()` helper (reads `.env` from the
