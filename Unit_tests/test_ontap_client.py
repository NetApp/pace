...
@pytest.fixture()
def client_with_mock_session() -> Generator[OntapClient, None, None]:
    client = OntapClient("10.0.0.1", "admin", "pass")
...
# ---------------------------------------------------------------------------
# OntapClient.update_auth
# ---------------------------------------------------------------------------


class TestUpdateAuth:
    def test_updates_session_auth(self) -> None:
        client = OntapClient("10.0.0.1", "admin", "oldpass")
        client.update_auth("newuser", "newpass")
        assert client._session.auth == ("newuser", "newpass")
        client.close()

    def test_replaces_original_credentials(self) -> None:
        client = OntapClient("10.0.0.1", "admin", "original")
        assert client._session.auth == ("admin", "original")
        client.update_auth("operator", "rotated")
        assert client._session.auth == ("operator", "rotated")
        client.close()


# ---------------------------------------------------------------------------
# OntapClient.wait_snapmirrored
# ---------------------------------------------------------------------------
...
class TestWaitSnapmirrored:
    ...
    def test_returns_immediately_when_snapmirrored(self) -> None:
        self.client.get = MagicMock(return_value={"state": "snapmirrored", "healthy": True})
        result = self.client.wait_snapmirrored("rel-uuid-1", interval=1, max_wait=60)
        assert result["state"] == "snapmirrored"
        assert self.client.get.call_count == 1
    ...
    def test_polls_until_snapmirrored(self) -> None:
        ...
        with patch("ontap_client.time.sleep"):
            result = self.client.wait_snapmirrored("rel-uuid-1", interval=1, max_wait=600)
        assert result["state"] == "snapmirrored"
        assert self.client.get.call_count == 3

    def test_raises_on_timeout(self) -> None:
        self.client.get = MagicMock(return_value={"state": "transferring"})
        with patch("ontap_client.time.sleep"):
            with pytest.raises(RuntimeError, match="Timed out"):
                self.client.wait_snapmirrored("rel-uuid-1", interval=2, max_wait=1)

    def test_queries_correct_relationship_url(self) -> None:
        self.client.get = MagicMock(return_value={"state": "snapmirrored"})
        self.client.wait_snapmirrored("my-rel-uuid", interval=1, max_wait=60)
        call_url = self.client.get.call_args[0][0]
        assert "my-rel-uuid" in call_url
...
class TestLoadEnvFile:
    def test_loads_key_value_pairs(
        ...
        from ontap_client import load_env_file
    ...
    def test_strips_surrounding_quotes(
        ...
        assert os.environ["SINGLE"] == "val"

    def test_existing_env_vars_not_overwritten(
        self, tmp_path: pytest.TempdirFactory, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        from ontap_client import load_env_file

        env_file = tmp_path / ".env"
        env_file.write_text("PRESET_KEY=file_value\n")
        monkeypatch.setenv("PRESET_KEY", "existing_value")
        load_env_file(str(env_file))
        import os

        assert os.environ["PRESET_KEY"] == "existing_value"
